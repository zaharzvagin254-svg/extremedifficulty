...
public class MobAIHandler {
    ...
    private static final double HEAR_RANGE       = 10.0;
    private static final double HEAR_RANGE_SNEAK = 4.0;
    ...
    public void onEntityJoin(EntityJoinLevelEvent event) {
        ...
        if (entity instanceof CaveSpider cs)              { addBasicGoals(cs, false); }
        else if (entity instanceof Husk husk)             { addZombieGoals(husk, false); }
        else if (entity instanceof Drowned drowned)       { addDrownedGoals(drowned); }
        else if (entity instanceof WitherSkeleton ws)     { addBasicGoals(ws, false); }
        else if (entity instanceof Zombie z && !(z instanceof ZombifiedPiglin)) { addZombieGoals(z, true); }
        else if (entity instanceof Stray stray)           { addArcherGoals(stray); }
        else if (entity.getClass() == Skeleton.class)     { addArcherGoals((Skeleton) entity); }
        else if (entity instanceof Spider sp)             { addBasicGoals(sp, false); }
        else if (entity instanceof Creeper cr)            { addBasicGoals(cr, false); }
        else if (entity instanceof Vindicator vin)        { addBasicGoals(vin, false); }
        else if (entity instanceof Pillager pil)          { addArcherGoals(pil); }
        else return;
    ...
    private void setFollowRange(Mob mob) {
    ...
    private void addZombieGoals(Zombie mob, boolean breakDoors) {
        // Replace vanilla targeting with our LOS-aware versions
    ...
    // -------------------------------------------------------------------------
    // SERVER TICK - enforce LOS every tick, not every 20 ticks
    // -------------------------------------------------------------------------
    ...
    /**
     * KEY METHOD: called every 10 ticks for every mob.
     * If mob has a player target but no LOS and can't hear - clear target.
     * This is the single reliable way to stop wall-targeting.
     * Works for ALL goals including HurtByTargetGoal.
     */
    private void enforceLOS(Mob mob) {
        if (!(mob.getTarget() instanceof Player player)) return;
        if (player.isCreative() || player.isSpectator()) return;

        // If mob has LOS - keep target
        if (hasReliableLOS(mob, player)) return;

        // No LOS - check hearing (non-sneaking player within range)
        if (!player.isCrouching()) {
            double hearR = player.isSprinting() ? HEAR_RANGE * 1.3 : HEAR_RANGE;
            if (mob.distanceToSqr(player) < hearR * hearR) return;
        }

        // No LOS and can't hear - lose target
        var tag = mob.getPersistentData();
        // Save last known position for search
        ...
        mob.setTarget(null);
        // Also clear HurtByTargetGoal memory so it doesn't immediately re-aggro
    ...
    private void applyLeadShotIfNeeded(Mob mob, long gt) {
        ...
        double dist = mob.distanceTo(player), t = dist / 1.5;
    ...
    private void updateSearchState(Mob mob, ServerLevel level, long gt) {
        ...
        // enforceLOS already handles active target - just update last known pos
        if (mob.getTarget() instanceof Player player) {
            if (gt % 20 == 0 && hasReliableLOS(mob, player)) {
                tag.putDouble(NBT_LAST_X, player.getX());
                tag.putDouble(NBT_LAST_Y, player.getY());
                tag.putDouble(NBT_LAST_Z, player.getZ());
        ...
        // During patrol: aggro only with reliable LOS
        if (tag.contains(NBT_LAST_X)) {
            Player nearby = level.getNearestPlayer(mob, DETECTION_RANGE);
    ...
    private void updateFlankData(Mob mob, long gt) {
        ...
        double len = Math.sqrt(dx*dx+dz*dz); if (len < 1.0) return;
        ...
        if (count == 0) { tag.putFloat(NBT_FLEE_DX,(float)ndx); tag.putFloat(NBT_FLEE_DZ,(float)ndz); }
            ...
            float a=0.3f, ax=tag.getFloat(NBT_FLEE_DX)*(1-a)+(float)(ndx*a);
    ...
    /**
     * Reliable LOS check with COLLIDER clip.
     * Offset 0.1 blocks toward target to avoid false-positive when mob is against wall.
     */
    static boolean hasReliableLOS(Mob mob, LivingEntity target) {
        ...
        if (len > 0.2) from = from.add(dir.normalize().scale(0.1));
    ...
    static boolean canHearPlayerPublic(Mob mob, Player player) {
        ...
        double r = player.isSprinting() ? HEAR_RANGE*1.3 : HEAR_RANGE;
        return mob.distanceToSqr(player) < r*r;
    }

    private boolean canHearPlayer(Mob mob, Player player) {
        return canHearPlayerPublic(mob, player);
    }
    ...
    static class DetectionGoal extends NearestAttackableTargetGoal<Player> {
        ...
        @Override public boolean canUse() {
            ...
            return hasReliableLOS(mob, nearest);
        }
    }
    ...
    static class SmartBreakDoorGoal extends BreakDoorGoal {
        ...
        @Override public boolean canUse(){
            ...
            return mob.getPersistentData().getByte(NBT_SAW_DOOR)==1&&super.canUse();
        }
    ...
    static class AdvancedSearchGoal extends Goal {
        ...
        private BlockPos target=null; private int lt=0,st=0,loiterTicks=0;
        private double lx=Double.MIN_VALUE,lz;
        private static final int STUCK=60,LOITER_MIN=40,LOITER_MAX=100;
        ...
        @Override public void start(){lt=0;st=0;loiterTicks=0;lx=mob.getX();lz=mob.getZ();mob.setAggressive(false);pick();}
        @Override public void tick(){
            ...
            if(loiterTicks>0){loiterTicks--;mob.getNavigation().stop();return;}
            if(target==null||mob.blockPosition().closerThan(target,2.0)){
                loiterTicks=LOITER_MIN+mob.getRandom().nextInt(LOITER_MAX-LOITER_MIN);pick();return;}
            if(lt%100==0)pick();
            if(target!=null){int state=mob.getPersistentData().getInt(NBT_SEARCH_STATE);
                mob.getNavigation().moveTo(target.getX()+0.5,target.getY(),target.getZ()+0.5,state==1?speed*0.7:speed*0.5);}
        }
        private void pick(){
            var tag=mob.getPersistentData();if(!tag.contains(NBT_LAST_X)){target=null;return;}
            double cx=tag.contains(NBT_ANCHOR_X)?tag.getDouble(NBT_ANCHOR_X):tag.getDouble(NBT_LAST_X);
            double cz=tag.contains(NBT_ANCHOR_Z)?tag.getDouble(NBT_ANCHOR_Z):tag.getDouble(NBT_LAST_Z);
            int state=tag.getInt(NBT_SEARCH_STATE);double r=state==1?6.0:12.0;
            if(mob.getRandom().nextFloat()<0.2f){
                double toX=cx-mob.getX(),toZ=cz-mob.getZ(),len=Math.sqrt(toX*toX+toZ*toZ);
                if(len>0.5){int nx=(int)(cx+(toX/len)*r),nz=(int)(cz+(toZ/len)*r);
                    int ny=mob.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,nx,nz);
                    if(ny>0&&Math.abs(ny-mob.getBlockY())<=5){BlockPos c=new BlockPos(nx,ny,nz);
                        var path=mob.getNavigation().createPath(c,1);
                        if(path!=null&&path.canReach()){target=c;return;}}}
            }
            for(int a=0;a<3;a++){
                double ang=mob.getRandom().nextDouble()*Math.PI*2,d=r*(0.3+mob.getRandom().nextDouble()*0.7);
                int nx=(int)(cx+Math.cos(ang)*d),nz=(int)(cz+Math.sin(ang)*d);
                int ny=mob.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,nx,nz);
        ...
        @Override public void stop(){mob.getNavigation().stop();mob.setAggressive(false);target=null;lt=0;st=0;loiterTicks=0;lx=Double.MIN_VALUE;}
    }
    ...
    static class MemoryPatrolGoal extends Goal {
        ...
        @Override public void stop(){mob.getNavigation().stop();mob.setAggressive(false);dest=null;pt=0;if(mob.level() instanceof ServerLevel sl)mob.getPersistentData().putLong(NBT_MEM_CD,sl.getGameTime()+MEMORY_PATROL_COOLDOWN);}
    }

    static class SkeletonCoverGoal extends Goal {
        private final Mob mob; private BlockPos coverPos=null;
        private int cooldown=0,atCoverTicks=0; private boolean movingToCover=false;
        ...
        private boolean hasBow(){var i=mob.getMainHandItem().getItem();return i instanceof BowItem||i instanceof CrossbowItem;}
        @Override public boolean canUse(){if(!hasBow()||mob.getTarget()==null)return false;if(cooldown>0){cooldown--;return false;}if(coverPos!=null&&mob.blockPosition().closerThan(coverPos,3.0)&&atCoverTicks<STAY)return false;return findCover(mob.getTarget());}
        @Override public boolean canContinueToUse(){return hasBow()&&mob.getTarget()!=null&&coverPos!=null&&atCoverTicks<STAY;}
        private boolean findCover(LivingEntity target){
            int mx=mob.getBlockX(),mz=mob.getBlockZ();double tx=target.getX(),tz=target.getZ();
            var level=mob.level();BlockPos best=null;double bestScore=-1;
            for(int i=0;i<16;i++){double angle=mob.getRandom().nextDouble()*Math.PI*2,dist=PMIN+mob.getRandom().nextDouble()*(PMAX-PMIN);
                int nx=mx+(int)(Math.cos(angle)*dist),nz=mz+(int)(Math.sin(angle)*dist);
                int ny=level.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,nx,nz);if(ny<=0)continue;
                BlockPos cand=new BlockPos(nx,ny,nz);double dtx=nx-tx,dtz=nz-tz,dtt=Math.sqrt(dtx*dtx+dtz*dtz);if(dtt<6||dtt>20)continue;
                double cq=evalCover(level,cand,target);if(cq<=0)continue;
                Vec3 eye=new Vec3(nx+0.5,ny+1.6,nz+0.5);var clip=level.clip(new ClipContext(eye,target.getEyePosition(),ClipContext.Block.COLLIDER,ClipContext.Fluid.NONE,mob));
                if(clip.getType()!=HitResult.Type.MISS)continue;
                var path=mob.getNavigation().createPath(cand,1);if(path==null||!path.canReach())continue;
                double ds=1.0-Math.abs(dtt-12.0)/8.0,score=cq*0.7+Math.max(0,ds)*0.3;if(score>bestScore){bestScore=score;best=cand;}}
        ...
        private double evalCover(Level level,BlockPos pos,LivingEntity target){
            double pdx=pos.getX()-target.getX(),pdz=pos.getZ()-target.getZ(),pl=Math.sqrt(pdx*pdx+pdz*pdz);if(pl<0.1)return 0;pdx/=pl;pdz/=pl;
            int[]ddx={1,-1,0,0},ddz={0,0,1,-1};int cover=0;
            for(int i=0;i<4;i++){BlockPos nb=pos.offset(ddx[i],0,ddz[i]);if(!level.getBlockState(nb).isSolidRender(level,nb))continue;if(ddx[i]*(-pdx)+ddz[i]*(-pdz)>0.3)cover++;}
            if(cover==0)return 0;boolean cs=level.getBlockState(pos.above()).isAir();if(!cs&&cover>=3)return 0.1;return 0.5+cover*0.15;
        }
        ...
        @Override public void tick(){atCoverTicks++;if(coverPos==null)return;if(mob.blockPosition().closerThan(coverPos,3.0)){if(movingToCover){mob.getNavigation().stop();movingToCover=false;}}else{movingToCover=true;if(atCoverTicks%20==0)mob.getNavigation().moveTo(coverPos.getX()+0.5,coverPos.getY(),coverPos.getZ()+0.5,1.0);}}
    ...
    static class SafeKeepDistanceGoal extends Goal {
        ...
        @Override public void tick(){LivingEntity t=mob.getTarget();if(t==null)return;double dx=mob.getX()-t.getX(),dz=mob.getZ()-t.getZ(),ls=dx*dx+dz*dz;if(ls<0.0001)return;double inv=1.0/Math.sqrt(ls),tx=mob.getX()+dx*inv*3,tz=mob.getZ()+dz*inv*3;int ty=mob.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,(int)tx,(int)tz);if(Math.abs(ty-mob.getBlockY())>4)return;mob.getNavigation().moveTo(tx,ty,tz,1.0);}
    }
}
